...
tdbc + tdbc::sqlite3, plus tcllibc on Linux, twapi on Windows) builds
and self-tests on both platforms (tls on win64 = static OpenSSL,
verified under wine incl. winstore certificate verification).
Interfaces (recipe fields, CLI) may change.
